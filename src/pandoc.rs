...
pub async fn run_postproc_filters(
	...
	frag_search_results(db, tera, ast, cookie).await
}
...
async fn include(db: &Pool<Postgres>, _tera: &Arc<RwLock<Tera>>, mut ast: Pandoc) -> Pandoc {
					...
					article
						...
						.filter_map(|b| {
							...
							match (taking, b) {
								...
								(Some(tl), Block::Header(level, (id, _, _), _))
									if tl <= level
										&& !include_spec
											.headings
											.iter()
